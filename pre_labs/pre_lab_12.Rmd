...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It returns the unique words that are in the set, as well as characters such as the comma.
```{r}
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It separates the unique words from the sentences, so you can just see what words are used.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** This is not particularly interesting, since the most common words he uses are the most common words in the English language.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Now it is more interesting. It shows Cardin's top priority when addressing the public is health.
...
**Task** Run the following code and describe the results. **Answer** Health falls down the list in 2024, but it is still a very common phrase for Senator Cardin.
...
**Task** Run the following code and describe the results. **Answer** We get pairs of words, with the most common word pairs at the top of the list.
...
**Task** Run the following code and describe the results. **Answer** Now we get health care, amount awarded and purpose funds.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** I think lemmatizing is the more effective option here, because we don't really care about the tense of each word.
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** The variations of the word "fund" are bundled together in the lemmatization.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** The word "rail" is listed as negative, which makes me a little nervous. Also, "critical" could be used in a positive way, like if a goalie makes a critical save.
