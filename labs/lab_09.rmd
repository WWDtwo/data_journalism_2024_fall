...
**A1.** The story is that more people are returning to voting on election day now that Covid is no longer a substantial issue. https://datawrapper.dwcdn.net/g6hwr/1/

```{r}
md_voting_methods <- read.csv("data/md_votes_by_type.csv")

md_voting_methods_comp <- md_voting_methods |>
  mutate(
    early_pct = Early/Total * 100,
    election_pct = Election.Day/Total * 100,
    mail_pct = Mail/Total * 100
  )

write_csv(md_voting_methods_comp, "data/md_voting_methods_comp.csv")
```

**Q2.** You've been assigned to track early-voting turnout in Maryland at the county level, because somebody hates you. To do that, use [data from the state] (https://elections.maryland.gov/press_room/2024_stats/PG24/EarlyVoting%20RAW%20data.csv). YOU DO NOT NEED TO LOAD THIS FILE. I've created several CSV files from it; each has a row for each day, with the counties as columns, so you can track early voting turnout in certain categories. The available files in the data folder are:
...
**A2.** https://datawrapper.dwcdn.net/pTUAV/1/ I picked the CSV files I did because combining men and women required the fewest .csv files.

```{r}
ev_totals_women <- read.csv("data/ev_totals_women.csv")
ev_totals_men <- read.csv("data/ev_totals_women.csv")

ev_totals <- full_join(ev_totals_men, ev_totals_women)

write_csv(ev_totals, "ev_totals.csv")
```
...
```{r}
md_winred_zips <- read.csv(md_winred_zip_totals.csv)
md_actblue_zips <- read.csv(md_actblue_zip_totals.csv)
```
