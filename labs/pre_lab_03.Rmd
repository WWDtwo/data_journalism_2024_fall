...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** The type of data it was looking for was not there, and instead there was something else, and there appear to be 10,000 points of issue
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** I do not see any
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** District is a double, precinct is a string, mail is also a double
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: Exactly 402,345 results have NA mailing address
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** The deceased category seems worthy of investigation, as do the felony categories, seeing as those two types of individuals are not supposed to be voting.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** It is a character/string
...
**Answer** There is a lack of consistency with the column naming, the amount column should be an integer and not a string, and the 14th and 7th rows are duplicates of each other
...
**Answer**There is now an x in front of the name
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** The amount argument needs to be an integer for the sum function to work.
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** dbl
...
**Task** Run the following codeblock to add up the amount of all contributions in this data. What is the total? Answer below. **Answer** 226 contributions
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** Mr. Derrick Hamilton
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** This dataframe now has 13 rows instead of 14
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** The last four digits have been dropped
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** Just one
